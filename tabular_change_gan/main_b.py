...
def run_one_epoch_lower_bound(iter, total_iter, train_encoder, cls_models_one, gan_model, eta, preprocessor_cluster, index, df_small, no_syn, no_syn_adj, Pg, delta1, delta2, file_txt, b_value):
    print('--------------------------------')
    ...
    df_optim, no_syn, no_syn_adj = choose_optim_data(result_diff_df, cfg.CLUSTER.SAVE_DIR_SYN_DATA, no_syn, no_syn_adj, cls_models_one, b_value)
    ...
    print(f"Lower bound calculation time: {time.time() - lower_bound_start:.2f} seconds")
...
def prepare_clustering(loaded_df_oracle, loaded_df_small, model, b_value=cfg.CLUSTER.ADJUST_FACTOR):
...
def run():  
    b_ls = [12, 15, 18, 0.5]
    for b in b_ls:
        loaded_df_train, loaded_df_small, loaded_df_oracle, X_train, y_train, X_small_test, y_small_test, X_oracle, y_oracle, train_encoder, pre_processor, cls_models, model, preprocessor_cluster, dim, index, centroids, eta, no_syn, df_small, df_oracle, Pg, no_syn_opt, no_syn_adj = prepare_process(b)
        ...
        for name, clf in tqdm(cls_models.items()):
            ...
            result_file = os.path.join(cfg.CLUSTER.TEMP_DIR, f"result_{b}_{name}.txt")
            ...
            for iter in range(cfg.LOWER_BOUND.NUM_ITERATIONS):
                ...
                result = run_one_epoch_lower_bound(iter, cfg.LOWER_BOUND.NUM_ITERATIONS, train_encoder, cls_models_one, model, eta, preprocessor_cluster, index, df_small, no_syn, no_syn_adj, Pg, cfg.LOWER_BOUND.DELTA_1, cfg.LOWER_BOUND.DELTA_2, result_file, b)
