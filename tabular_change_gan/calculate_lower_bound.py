...
def calculate_Boostrap_loss(g_adj, df_small_test, encoder, delta1, delta2, clf):
    # Reduce number of bootstrap iterations to a reasonable number
    n_boot = min(cfg.LOWER_BOUND.NUM_BOOTSTRAP_ITERATIONS, g_adj)  # Limit to 1000 iterations
    # n_boot = g_adj
    K = len(df_small_test)
    
    print(f"Using {n_boot} bootstrap iterations")
    
    # Pre-compute all possible random indices
    all_indices = np.random.choice(K, size=(n_boot, K), replace=True)
    
    # Pre-process the data once
    X_original, y_original = split_to_X_y(df_small_test, type='test', encoder=encoder)
    ...
    batch_size = min(100, n_boot)  # Process 100 iterations at a time
    n_batches = (n_boot + batch_size - 1) // batch_size
    
    # Split indices into batches
    batches = [all_indices[i:i + batch_size] for i in range(0, n_boot, batch_size)]
    
    # Process batches in parallel
    losses = Parallel(n_jobs=n_cores)(
        delayed(process_batch)(batch, X_original, y_original, clf)
    ...
    delta = delta1 + delta2
    loss = np.percentile(losses, delta * 100)  # Convert to percentage
    
    # results[name] = {'L01': loss}
    # print(f"Loss for {name}: {loss:.4f}")
    # df = pd.DataFrame(results)
    # df.to_csv('Datasets/Loss_data/loss_boot.csv', index=False)
    return loss
